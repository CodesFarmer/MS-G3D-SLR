...
def get_parser():
    ...
    parser.add_argument(
        '--prefix',
        default=None,
        help='the weights prefix for network testing multi epoches mode')
    parser.add_argument(
        '--ignore-weights',
        type=str,
        default=[],
...
class Processor():
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                dataset=Feeder(is_test=True, **self.arg.train_feeder_args),
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            if self.arg.prefix is not None:
                for eph in range(70, 85, 5):
                    self.arg.weights = "%s_%d.pth" % (self.arg.prefix, eph)
                    self.load_model()
                    self.eval(
                        epoch=0,
